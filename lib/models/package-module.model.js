...
class PackageModulesModel {
  ...
  constructor({ directoryName, folderPath, processPath }) {
  ...
  async installDependencies() {
    ...
    await execPromise(
      `cd ${this.#folderPath} && npm i -D ${this.dependencies.join(' ')}`
    );
  ...
  async removePackage() {
    await fs.unlinkSync(this.folderPath + '/package.json');
    await fs.unlinkSync(this.folderPath + '/package-lock.json');
  }

  async removeDependencies() {
    await execPromise(`cd ${this.folderPath} && npm uninstall ${this.dependencies.join(' ')}`);
  }

  async removeNodeModules() {
    await fs.removeSync(this.folderPath + '/node_modules');
  }
}
